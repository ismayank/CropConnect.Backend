...
module.exports.controller = (app, io, socket_list) => {
    ...
    app.post('/api/admin/login', (req, res) => {
        ...
        helper.CheckParameterValid(res, reqObj, ["email", "password", "device_token"], () => {
            ...
            db.query("UPDATE `user_detail` SET `auth_token` = ?, `device_token` = ?, `modify_date` = NOW() WHERE `user_type` = ? AND `email` = ? AND `password` = ? AND `status` = ? ", [authToken, reqObj.device_token, "2", reqObj.email, reqObj.password, "1"], (err, result) => {
    ...
    app.post('/api/admin/brand_add', (req, res) => {
        ...
        // Check if the access token is provided in headers
        if (!req.headers['access_token'] ) {
            return res.json({ "status": "0", "message": "Missing access token" });

        }
        console.log("Access Token from headers:", req.headers['access_token']);
    
        // Check if the required parameter is present
        helper.CheckParameterValid(res, reqObj, ["brand_name"], () => {
            // Proceed with access token check and database logic
            checkAccessToken(req.headers, res, (uObj) => {
                ...
                db.query("SELECT `brand_id`, `brand_name` FROM `brand_detail` WHERE `brand_name` = ? AND `status` = ?", [reqObj.brand_name, "1"], (err, result) => {
                    ...
                    if (result.length > 0) {
                        ...
                        console.log("Brand already added:", result[0]);
                        res.json({ "status": "1", "payload": result[0], "message": "Brand already added" });
                    } else {
                        ...
                        db.query("INSERT INTO `brand_detail` (`brand_name`, `created_date`, `modify_date`) VALUES (?, NOW(), NOW())", [reqObj.brand_name], (err, result) => {
                            ...
                            if (result) {
                                ...
                                res.json({ "status": "0", "message": "Failed to add brand" });
                            }
                        });
                    }
                });
            ...
            form.parse(req, (err, reqObj, files) => {
                ...
                helper.CheckParameterValid(res, reqObj, ["name", "detail", "cat_id", "brand_id", "type_id", "unit_name", "unit_value", "nutrition_weight", "price", "nutrition_data"], () => {
                    helper.CheckParameterValid(res, files, ["image"], () => {
                        ...
                        db.query("INSERT INTO `product_detail`(`cat_id`, `brand_id`, `type_id`, `name`, `detail`, `unit_name`, `unit_value`, `nutrition_weight`, `price`, `created_date`, `modify_date`) VALUES (?,?,?,?,?,?,?,?,?, NOW(), NOW())", 
                        [reqObj.cat_id[0], reqObj.brand_id[0], reqObj.type_id[0], reqObj.name[0], reqObj.detail[0], reqObj.unit_name[0], reqObj.unit_value[0], reqObj.nutrition_weight[0], reqObj.price[0]], 
                        (err, result) => {
                            if (err) {
                                helper.ThrowHtmlError(err, res);
                                return;  // Prevent further execution after sending response
                            ...
                            if (result) {
                                ...
                                if (nutritionDataArr.length > 0) {
                                    db.query("INSERT INTO `nutrition_detail`(`prod_id`, `nutrition_name`, `nutrition_value`) VALUES ?", [nutritionInsertData], (err, nResult) => {
                                        ...
                                        if (nResult) {
                                            helper.Dlog("nutritionInsert success");
                                        } else {
                                            helper.Dlog("nutritionInsert fail");
                                        }
                                    });
                                ...
                                // Send the response after everything is inserted
                                if (!res.headersSent) {
                                    res.json({
                                        "status": "1",
    ...
    app.post('/api/admin/product_list', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            db.query(
                "SELECT `pd`.`prod_id`, `pd`.`cat_id`, `pd`.`brand_id`, `pd`.`type_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`, `pd`.`created_date`, `pd`.`modify_date`, `cd`.`cat_name`, " +
                "IFNULL(`bd`.`brand_name`, '') AS `brand_name`, `td`.`type_name`, " +
                "(CASE WHEN GROUP_CONCAT(`imd`.`image`) IS NOT NULL THEN CONCAT('" + image_base_url + "', GROUP_CONCAT(`imd`.`image`)) ELSE '' END) AS `image`, " +
                "IFNULL(MAX(`od`.`price`), `pd`.`price`) AS `offer_price`, " +
                "AVG(CASE WHEN `rd`.`rate` IS NOT NULL THEN `rd`.`rate` ELSE 5.0 END) AS `avg_rating` " +
    ...
    app.post('/api/admin/promo_code_update', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (userObj) => {
            helper.CheckParameterValid(res, reqObj, ["promo_code_id", "title", "description", "type", "min_order_amount", "max_discount_amount", "offer_price", "start_date", "end_date"], () => {
                ...
                db.query("UPDATE `promo_code_detail` SET `title`= ? ,`description`= ? ,`type`= ? ,`min_order_amount`= ? ,`max_discount_amount`= ? ,`offer_price`= ? ,`start_date`= ? ,`end_date`= ?, `modify_date` = NOW() WHERE `promo_code_id` = ? AND `status` = 1 ", [
