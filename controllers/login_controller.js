...
module.exports.controller = (app, io, socket_list) => {
    ...
    app.post('/api/app/login', (req, res) => {
        ...
        helper.CheckParameterValid(res, reqObj, ["email", "password", "device_token"], () => {
            ...
            // Update user details with auth_token and device_token
            db.query("UPDATE `user_detail` SET `auth_token`= ?, `device_token`=?, `modify_date`= NOW() WHERE `email` = ? AND `password` = ? AND `status` = '1'", 
                [auth_token, reqObj.device_token, reqObj.email, reqObj.password], (err, result) => {
                if (err) {
                ...
                if (result.affectedRows > 0) {
                    // Select user details
                    db.query('SELECT `user_id`, `username`, `user_type` , `name`, `email`, `mobile`, `mobile_code`, `password`, `auth_token`, `device_token`, `reset_code`, `status`, `created_date`, `modify_date` FROM `user_detail` WHERE `email` = ? AND `password` = ? AND `status` = "1"', 
                        [reqObj.email, reqObj.password], (err, result) => {
                        if (err) {
                            helper.ThrowHtmlError(err, res);
                            return;
                        }
...
function checkAccessToken(headerObj, res, callback, require_type = "") {
    ...
    if (!headerObj.access_token) {
        res.json({ "status": "0", "message": "Missing parameter (access_token)" });
        return;
    }
    ...
    helper.CheckParameterValid(res, headerObj, ["access_token"], () => {
        db.query(
            "SELECT `user_id`, `username`, `user_type`, `name`, `email`, `mobile`, `mobile_code`, `auth_token`, `device_token`, `status` FROM `user_detail` WHERE `auth_token` = ? AND `status` = ?",
